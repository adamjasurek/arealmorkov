...
function DrinkSectionTable({ section }: { section: DrinkSection }) {
  ...
  return (
    <div className="card-brutal p-4 sm:p-5 md:p-6">
      <h4 className="font-display text-2xl text-gold-gradient">{section.title}</h4>
      <div className="-mx-1 overflow-x-auto">
        <table className="mt-4 w-full min-w-[16rem] border-collapse font-sans text-sm">
          {dualSize ? (
            <thead>
              <tr className="border-b-2 border-foreground/30 text-muted">
                <th className="pb-2 text-left font-medium" />
                <th className="pb-2 text-right font-medium whitespace-nowrap">0,5l</th>
                <th className="pb-2 text-right font-medium whitespace-nowrap">0,3l</th>
              </tr>
            </thead>
          ) : null}
          <tbody>
