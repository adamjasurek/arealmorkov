...
import type { MenuPriceItem, PizzaItem } from '@/data/menu'
...
export function FoodMenu() {
  return (
    <div>
      <h3 className="font-display text-4xl text-gold-gradient md:text-5xl">{foodMenuTitle}</h3>
      <p className="mt-2 font-sans text-muted">Restaurace Podhora · Pizza Olymp</p>

      <div className="mt-8 grid gap-6 xl:grid-cols-3">
        <PizzaTable title="Pizza" subtitle="Rajčatový základ" items={tomatoPizzas} />

        <div className="flex flex-col gap-6">
          <PizzaTable title="Smetanový základ" items={creamPizzas} />
          <div className="card-brutal p-5 md:p-6">
            ...
            <p className="mt-4 font-sans text-sm text-muted">{foodMenuNotes.extraIngredients}</p>
          </div>
        </div>

        <div className="card-brutal flex h-full flex-col p-5 md:p-6">
          <SimpleMenuTable title="Další nabídka" items={otherOfferings} />
          <div className="mt-6 border-t-2 border-foreground/20 pt-6">
            <SimpleMenuTable title="Smažené sýrečky" items={friedCheeses} />
          </div>
          <p className="mt-6 font-sans text-sm text-muted">{foodMenuNotes.allergens}</p>
        </div>
      </div>
    </div>
  )
